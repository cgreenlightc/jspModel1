...
public class BbsDto implements Serializable {
		...
		private int ref;	// 답글용 변수 : 	그룹번호(글번호)
		private int step;	//				행번호
		private int depth;	//				깊이
		
		private String title;	// 글의 제목
		private String content;	// 글의 내용
		private String wdate;	// 글의 등록일
		
		private int del;		// 글이 삭제됐는지 안됐는지 가려주는
		private int readcount;	// 조회수
		
		public BbsDto() {
		}

		public BbsDto(String id, String title, String content) {   // 외부에서 들어오는 용. 글쓴이가 따로 쓰는 부분(작성자용)
			super();
			this.id = id;
			this.title = title;
		...
		public String toString() {
			return "BbsDto [seq=" + seq + ", id=" + id + ", ref=" + ref + ", step=" + step + ", depth=" + depth
